...
export const CONCEPTS: Concept[] = [
  ...
  },
  {
    ...
    definition:
      "A half step (or semitone) is the smallest move in Western music: from any key to the one immediately beside it, black or white. A whole step (or whole tone) is two half steps stacked, so you skip the key in between. Both are distances between notes, not kinds of note — every key is a full note, so an octave holds twelve notes and twelve half steps, and 'whole' just means the larger of the two common step sizes. (The similar-sounding whole note and half note are rhythm terms for how long a note lasts.) Every scale is a recipe written in these two distances — the major scale, for instance, runs whole–whole–half–whole–whole–whole–half. The catch is on the white keys: most neighbours are a whole step apart, but E–F and B–C are half steps with no black key between them, which is exactly why the piano is laid out the way it is.",
...
export function searchConcepts(
  ...
  let items: readonly Concept[] = CONCEPTS;
  if (filters.genre) {
    items = items.filter((concept) => concept.genres?.includes(filters.genre!));
  }
  return sortByLabel(
    filterByHaystack(
      items,
      query,
      (concept) => HAYSTACKS.get(concept.slug) ?? "",
    ),
