...
| Synth v2 range too strict on tablet | [src/instruments/synth/v2/SynthV2.tsx](src/instruments/synth/v2/SynthV2.tsx) | `/synth/v2` auto-shrinks to one octave on tablet even when two would fit. Lesson keyboards already show two octaves and look fine. The fit threshold (`MIN_WHITE_KEY_PX`) is too conservative — loosen it so two octaves stay up when there is room. | **Parked (2026-09-14).** Capture only; plan later. | not specified |
| Synth v2 Type dropdown: aliases without clutter | [src/instruments/synth/v2/SynthV2.tsx](src/instruments/synth/v2/SynthV2.tsx) | The Type dropdown is missing names people look for (e.g. Freygish, Ukrainian Dorian) even when the scale is in the catalog under another label (Freygish = Phrygian dominant). Show every catalog option and its alternate names in that dropdown, without polluting the list. How to surface aliases is undecided. | **Parked (2026-09-14).** Capture only; plan later. | not specified |
...
| Genre Lab | plan | Phase 0 content research (lock reggae one-drop, rock backbeat, blues shuffle), then Phase 1 + `GroovePlayer` + `GrooveGrid` + `BeatComparer` + Reggae page | suggested first milestone in-doc | not specified |
...
| Synth v2 range too strict on tablet | fix | `/synth/v2` drops to one octave on tablet when two would fit; lesson pianos already show two. Loosen the fit cap. | plan later | not specified |
| Synth v2 Type dropdown: aliases without clutter | fix | Surface every catalog scale and its alternate names in the Type dropdown (Freygish, Ukrainian Dorian, …) without polluting the list. | plan later | not specified |
