...
| Scales catalog + lesson roadmap | [docs/plans/scales-catalog-and-lessons.md](docs/plans/scales-catalog-and-lessons.md) | One `SCALE_CATALOG` + `spellScale` in `src/lib/music`, a lesson template, and the ordered list of `/scales/[slug]` pages (major → pentatonic → harmonica modes → harmonic-minor family incl. freygish → fill → double harmonic / Rast) | **Klezmer track built, awaiting review (2026-09-11).** PR #4: catalog (Phase 0), lesson primitives + comparer modes, and `/scales/harmonic-minor`, `/scales/freygish`, `/scales/ukrainian-dorian` live. Paused on the Phase 6 Freygish tone sign-off before Phases 1 (major, minor pentatonic pages), 2, 4, 5. | not specified |
